...
class Preferences
  ...
  # The distinct provider+model pairs the user has already configured, harvested
  # from the saved model sets (both halves of each set) and the active
  # provider/worker. Each entry: { provider:, model:, name: }, name may be nil.
  def self.configured_pairs
    data = read_raw
    rows = model_sets(data).flat_map do |entry|
      [[entry['provider'], entry['model'], entry['name']],
       [entry['worker_provider'] || entry['provider'], entry['worker_model'], nil]]
    end
    rows << [data['provider'], data['model'], nil]
    rows << [data['worker_provider'] || data['provider'], data['worker_model'], nil]

    rows.each_with_object({}) do |(provider, model, name), pairs|
      next if provider.nil? || model.to_s.empty?

      pair = pairs[[provider.to_s, model.to_s]] ||= { provider: provider.to_s, model: model.to_s, name: nil }
      pair[:name] ||= name
    end.values
  end
  ...
  def self.generate_model_set_name(_provider_id, model_id)
    m = model_id.to_s
